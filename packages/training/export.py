"""Export float32 qualification and quantized candidate WebGPU models."""
...
def serialize(model: Tagger, *, quantized: bool, bits: int = 8) -> tuple[bytes, Tagger, float]:
    if bits not in range(2, 9):
        raise ValueError("Quantization bits must be between 2 and 8")
    bound = 2 ** (bits - 1) - 1
    ...
    for name in WEIGHTS:
        ...
        scale = max(float(value.abs().max()) / bound, 1e-12)
        code = (value / scale).round().clamp(-bound, bound).to(torch.int8)
...
def export_model(model: Tagger, folder: Path, fixtures, *, bits: int = 8) -> float:
    folder.mkdir(parents=True, exist_ok=True)
    for suffix, quantized in [(".f32", False), ("", True)]:
        blob, deployed, max_error = serialize(model, quantized=quantized, bits=bits)
...
def main(argv=None):
    ...
    ap.add_argument("--bits", type=int, choices=range(2, 9), default=5)
    ...
    max_error = export_model(model, folder, fixtures, bits=args.bits)
    _, deployed, _ = serialize(model, quantized=True, bits=args.bits)
    ...
    report = dict(
        format=f"GPA3 ordered-byte-conv32-scan128-v3-seven int{args.bits}; float32 qualification sibling",
        ...
        deployed_exact=deployed_result["exact_accuracy"],
    ...
    (folder / f"int{args.bits}-dev.json").write_text(
        json.dumps(deployed_result, indent=2, ensure_ascii=False)
    )
    print(
