...
def hf_row(record, country):
    ...
    for word, tag in zip(words, tags):
        ...
        if label == previous:
            ...
            spans[-1]["raw"] = text[spans[-1]["start"] : word.end()]
        else:
            spans.append(dict(label=label, start=word.start(), end=word.end(), raw=word.group()))
        previous = label
    return dict(
        text=text,
        components=spans,
        ...
        label_provenance="mapped-existing-tags",
        offset_basis="unicode-code-point",
        status=None,
    )
...
def gnaf_row(record):
    ...
    def number(prefix):
        value = numeric(record.get(prefix))
        return (
    ...
    if last and not numeric(record.get("number_last")):
        raise ValueError("range-end-designator-without-number")
    if last and not first:
        raise ValueError("range-end-without-start")
    lot = number("lot_number")
    locator = first + ("-" + last if last else "") if first else ("LOT " + lot if lot else "")
    postcode = numeric(record["postcode"])
    postcode = postcode.zfill(4) if postcode else ""
    ...
    row.update(
        omitted_source_fields=[
            field
            for field in ["lot_number", "lot_number_prefix", "lot_number_suffix"]
            if first and record.get(field) not in (None, "")
        ],
        source_fields=record,
        source_entity_group=record["address_detail_pid"],
        source_street_group=record["street_locality_pid"],
    )
    return row
...
def build(directory):
    ...
    for row in selected:
        ...
        for changed in variants:
            ...
            if (
                item is None
                or components(item[1], item[2], changed["text"]) != item[3]["components"]
            ):
...
def acquire(item, folder):
    ...
    if path.exists() and meta.exists():
        ...
        if record.get("url") != url or file_sha256(path) != record["sha256"]:
    ...
    try:
        with urllib.request.urlopen(
            ...
            with temporary.open("wb") as stream:
                while chunk := response.read(min(1024**2, limit + (not prefix) - size)):
                    size += len(chunk)
                    if size > limit:
