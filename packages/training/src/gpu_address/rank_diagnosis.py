...
def main(argv=None):
    ...
    with torch.no_grad():
        for start in range(0, len(data), 64):
            ...
            for i, (path, (_, _, offsets, row)) in enumerate(zip(first, rows)):
                truth = field_map(row["components"])
                rank = 1 if field_map(components(path, offsets, row["text"])) == truth else None
                if rank is None:
                    paths = top_paths(
                    ...
                    for j, (alternative, _) in enumerate(paths[1:], 2):
                        if field_map(components(alternative, offsets, row["text"])) == truth:
                            rank = j
                            if len(recovered) < 20:
                                recovered.append(
                                    dict(
    ...
    report = dict(
        ...
        scope="Oracle best-of-k label paths under public field-map metric; uses gold to choose. "
