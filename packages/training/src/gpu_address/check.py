...
from .prepare import identity, prefix_text, tagged, validate, variant, web_rows
...
def main(argv=None):
    ...
    for i in range(30):
        ...
        assert components(item[1], item[2], changed["text"]) == changed["components"]
    combinations = {
    ...
    item = encode(chinese)
    assert components(item[1], item[2], chinese["text"]) == chinese["components"]
    ...
    assert components(with_gaps[1], with_gaps[2], chinese["text"]) == chinese["components"]
