...
from .schema import FIELDS, Component
...
def tagged(line):
    ...
    for pair in body.split():
        ...
        if label in ("SEP", "FSEP"):
            pieces.append((",", None))
        elif label in FIELDS and token:
...
def main(argv=None):
    ...
    for record in csv.DictReader(io.StringIO((raw / "senzing-benchmark.raw").read_text())):
        components = [dict(label=k, raw=record[k]) for k in FIELDS if record.get(k)]
        row = dict(
    ...
    # A review queue is not a locked gold set; no teacher label masquerades as human truth.
    review = sorted(real, key=lambda row: digest(row["id"]))[:200]
