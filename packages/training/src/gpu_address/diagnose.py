...
def main(argv=None):
    ...
    if args.source_dev:
        ...
        result = evaluate(model, rows, "cpu")
        result.update(
            rejected=rejected,
    ...
    with torch.no_grad():
        for start in range(0, len(public), 64):
            ...
            for i, (path, (_, _, offsets, row)) in enumerate(zip(paths, rows)):
                predicted = field_map(components(path, offsets, row["text"]))
                truth = field_map(row["components"])
                exact = predicted == truth
                oracle_exact = baseline[row["id"]]["exact"]
                counts["rows"] += 1
                counts["exact"] += exact
                counts[f"student_{exact}_teacher_{oracle_exact}"] += 1
                greedy = field_map(components(argmax[i][: len(path)], offsets, row["text"]))
