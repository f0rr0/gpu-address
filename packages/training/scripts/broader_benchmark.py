...
def collect():
    ...
    if not (DATA/'companies.zip').exists():
        with urllib.request.urlopen(url, timeout=60) as response, (DATA/'companies.zip').open('wb') as out:
            while chunk := response.read(1024*1024):
                out.write(chunk)
    buckets, rejected = defaultdict(list), Counter()
    with zipfile.ZipFile(DATA/'companies.zip') as archive, archive.open(archive.namelist()[0]) as stream:
        ...
        for values in reader:
            ...
            if not postcode or not address['RegAddress.PostTown'] or not address['RegAddress.AddressLine1']:
            ...
            nation = 'Northern Ireland' if area=='BT' else 'Scotland' if area in scotland else 'Wales' if area in {'CF','LL','NP','SA','LD'} else 'England'
    ...
    candidates = {k:[dict(id=i,source_fields=a) for _,i,a in sorted(v,reverse=True)] for k,v in buckets.items()}
    ...
    def download(index):
        ...
        if path.exists():
            return
        params = dict(f='json',objectIds=','.join(map(str,identifiers[index*75:(index+1)*75])),outFields='OBJECTID,AddNo_Full,StNam_Full,Building,Floor,Unit,Room,SubAddress,LandmkName,Post_City,Inc_Muni,Uninc_Comm,State,Zip_Code,Addr_Type,NAD_Source',returnGeometry='false')
        with urllib.request.urlopen(base+'?'+urllib.parse.urlencode(params),timeout=40) as response:
            raw=response.read()
...
def prepare_baseline():
    if (PUBLIC / 'manifest.json').exists():
        raise ValueError('Frozen benchmark exists; use a new version, never overwrite its selection')
    blocked = set()
    ...
    for path in (ROOT/'data/website-benchmark/inputs.json', ROOT/'data/geosearch-sample-20260916/inputs.json', ROOT/'apps/website/public/evaluation/inputs.json'):
    ...
    for path in sorted(DATA.glob('nad-[0-9]*.json')):
        for item in json.loads(path.read_text())['features']:
            r = item['attributes']
            required = [r.get(k) for k in ('AddNo_Full', 'StNam_Full', 'Post_City', 'State', 'Zip_Code')]
            if not all(required) or any(re.search(r'not stated|unknown|<null>|unincorporated', str(v), re.I) for v in required):
            ...
            sub = ' '.join(f'{label} {r[k]}' for k,label in [('Building','Building'),('Floor','Floor'),('Unit','Unit'),('Room','Room')] if r.get(k))
            sub = sub or r.get('SubAddress') or ''
            street = ' '.join([r['AddNo_Full'], r['StNam_Full'], sub]).strip()
            row = external.render('nad:'+str(r['OBJECTID']), 'us', r['State'], [('street_address',street),('city',r['Post_City']),('state',r['State']),('postcode',r['Zip_Code'])], path.name)
    ...
    for nation, rows in candidates.items():
        for item in rows:
            ...
            if any(external.key(r['PostCode']) in external.key(v) for v in lines):
                ...
                continue
            values = [('street_address',r.get('POBox','')),('street_address',r['AddressLine1']),('street_address',r['AddressLine2']),('city',r['PostTown']),('postcode',r['PostCode'])]
    ...
    for stratum, rows in sorted(pool.items()):
        ...
        if len(rows)<count:raise ValueError(f'Not enough eligible rows: {stratum}: {len(rows)}')
    ...
    manifest = dict(protocol_version=3, frozen_at=datetime.now(timezone.utc).isoformat(),seed=SEED,
        rows=len(selected),entities=len(selected),cohorts=['complete'],
        selected=dict(Counter(r['stratum'] for r in selected)),eligible={k:len(v) for k,v in pool.items()},exclusions=dict(excluded),
        inputs_sha256=harness.sha(PUBLIC/'inputs.json'),preparation_sha256=harness.sha(Path(__file__)),
        model_sha256=harness.sha(ROOT/'packages/core/model.bin'),checkpoint_sha256=harness.sha(ROOT/'runs/ordered-h128-english-seven-20260916/best.pt'),
        ...
        sources={name:dict(sha256=harness.sha(DATA/name)) for name in ['companies.zip','companies-candidates.json','nad-max.json',*sorted(p.name for p in DATA.glob('nad-[0-9]*.json'))]},
        source_urls={'companies.zip':'https://download.companieshouse.gov.uk/BasicCompanyDataAsOneFile-2026-09-01.zip','nad':'https://services.arcgis.com/xOi1kZaI0eWDREZv/ArcGIS/rest/services/Address_Points_from_National_Address_Database_view/FeatureServer/0/query'},
...
def prepare():
    ...
    for row in rows:
        if row['country']=='gb':
            ...
            values.extend([('state',source.get('RegAddress.County','')),('postcode',source['RegAddress.PostCode'])])
            row = dict(row, **external.render(row['id'],row['country'],row['stratum'],values,row['source']))
    ...
    manifest.update(protocol_version=4, frozen_at=datetime.now(timezone.utc).isoformat(),
        inputs_sha256=harness.sha(PUBLIC/'inputs.json'),preparation_sha256=harness.sha(Path(__file__)),
    ...
    print('Frozen',len(revised),'addresses; UK with county:',sum(r.get('has_county',False) for r in revised))


if __name__=='__main__':
    ap=argparse.ArgumentParser(description=__doc__)
    ap.add_argument('mode',choices=['collect','prepare-baseline','prepare','gpu','libpostal','senzing','deep','report'])
    ...
    if mode=='collect':collect()
        ...
        if mode=='report':
            ...
            for model in external.MODELS:
                (predictions_dir/f'{model}.json').write_bytes((DATA/f'{model}-v4.json').read_bytes())
            external.DATA,external.PUBLIC=predictions_dir,PUBLIC
            external.report(rows,manifest)
        else:
            result=harness.deep(rows) if mode=='deep' else harness.gpu(rows) if mode=='gpu' else harness.postal(rows,mode)
            (DATA/f'{"deepparse" if mode=="deep" else mode}-v4.json').write_text(json.dumps(result,indent=2))
