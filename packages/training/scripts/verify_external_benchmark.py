...
def main():
    ...
    ap.add_argument(
        "--directory",
        default="evaluation-v4",
        choices=["evaluation", "evaluation-v3", "evaluation-v4"],
    )
    ...
    if manifest.get("protocol_version", 0) >= 3:
        ...
        for row in rows:
            ...
            if row["country"] == "gb":
                expected = [
                    ("street_address", source.get("RegAddress." + k, ""))
                    for k in ("POBox", "AddressLine1", "AddressLine2")
                ]
                ...
                sub = (
                    " ".join(
                        f"{k} {source[k]}"
                        for k in ("Building", "Floor", "Unit", "Room")
                        if source.get(k)
                    )
                    or source.get("SubAddress")
                    or ""
                )
                expected = [
                    ("street_address", f"{source['AddNo_Full']} {source['StNam_Full']} {sub}"),
                    ("city", source["Post_City"]),
                    ("state", source["State"]),
    ...
    for row in rows:
        ...
        if row["cohort"] != "complete":
            ...
            assert {p["label"]: p["raw"] for p in row["components"]} == expected
    supplement_path = public / "usaddress-results.json"
    if supplement_path.exists():
        supplement = json.loads(supplement_path.read_text())
        assert supplement["inputs_sha256"] == manifest["inputs_sha256"]
        assert supplement["rows"] == sum(r["country"] == "us" for r in rows)
        report["prediction_sha256"]["usaddress"] = supplement["prediction_sha256"]
        for group in report["groups"]:
            if group["country"] == "us" and group["cohort"] == "complete":
                group["models"]["usaddress"] = supplement["score"]
    for model, expected_hash in report["prediction_sha256"].items():
        ...
        pred = json.loads(raw)
        assert set(pred) == {r["id"] for r in rows if model != "usaddress" or r["country"] == "us"}
        for group in report["groups"]:
            if model == "usaddress" and group["country"] != "us":
                continue
            subset = [
                r
                for r in rows
                if r["country"] == group["country"]
                and (
                    r.get("has_county") is True
                    if group["cohort"] == "with-county"
                    else r.get("has_county") is False
                    if group["cohort"] == "without-county"
                    else r["cohort"] == group["cohort"]
                )
            ]
            ...
            for row in subset:
                ...
                for label in actual.keys() | gold.keys():
                    ...
                    breakdown[label]["correct"] += (
                        label in actual and label in gold and actual[label] == gold[label]
                    )
            ...
            for label, counts in breakdown.items():
                ...
                if "f1" in score["fields"].get(label, {}):
                    assert (
                        abs(
                            score["fields"][label]["f1"]
                            - 200 * counts["correct"] / (counts["rows"] + counts["predicted"])
                        )
                        < 1e-10
                    )
            ...
            assert all(
                abs(a - b) < 1e-9
                for a, b in zip(
                    score["interval"], [100 * (center - radius), 100 * (center + radius)]
                )
            )
    ...
    print(
        f"Verified {len(rows):,} inputs, {len(originals):,} source entities, prediction hashes, confidence intervals, and published overall/per-field scores."
    )
