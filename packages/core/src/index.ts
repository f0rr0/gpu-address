...
export async function createParser(
  ...
  void device.lost.then(info => {
    loss = new Error(`WebGPU device lost (${info.reason})${info.message ? `: ${info.message}` : ''}. Create a new parser.`);
  });
  const assertDevice = () => { if (loss) throw loss; };
  try {
    ...
    const infer = async (text: string): Promise<ParseResult> => {
      ...
      try {
        ...
        assertDevice();
        flat = new Float32Array(readback.getMappedRange(0, byteLength).slice(0));
      } catch (error) {
        assertDevice();
        throw error;
      } finally {
